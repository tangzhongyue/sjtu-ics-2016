#515030910312 唐仲乐
# Add IIADDL for correctness and save the cost on "irmovl+addl".
# Change the prediction of PC so that only when f_valP > f_valC will f_predPC choose f_valC,otherwise choose f_valP
# Avoid unnecessary stall caused by load/use hazard when the instructions are "mrmovl+rmmovl"
#/* $begin pipe-all-hcl */
####################################################################
...
int f_pc = [
	...
	M_icode == IJXX && !M_Cnd && M_ifun != 0 && M_valE<M_valA : M_valA;
	M_icode == IJXX && M_Cnd && M_ifun != 0 && M_valE>=M_valA : M_valE;
...
int f_predPC = [
	f_icode == ICALL || f_icode == IJXX && f_valP > f_valC && f_ifun != 0|| f_icode == IJXX && f_ifun == 0: f_valC;
	1 : f_valP;
];
...
int aluA = [
	...
	E_icode in { IIRMOVL, IRMMOVL, IMRMOVL, IIADDL, IJXX } : E_valC;
...
bool D_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd && E_ifun != 0 && E_valA > E_valC ) || 
	(E_icode == IJXX && e_Cnd && E_ifun != 0 && E_valA <= E_valC ) ||
	# Stalling at fetch while ret passes through pipeline
