...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    sign=(val<0);
    switch (op){
	case A_XOR: case A_AND:
	break;
	case A_ADD:
	ovf=(((argA < 0) == (argB < 0)) && ((val < 0) != (argA < 0)));break;
	case A_SUB:
	ovf=(((argB < 0) == (argA > 0)) && ((val < 0) != (argB < 0)));break;
	default:return DEFAULT_CC;break;
    }
...
stat_t nexti(y86sim_t *sim)
{
    ...
    next_pc++;
    regid_t rega=0;regid_t regb=0;long_t imm=0;
    /* get registers if needed (1 byte) */
    if(((icode>=2) && (icode<=6)) || ((icode>=0xA) && (icode <=0xB))){
	get_byte_val(sim->m, next_pc, &codefun);
    ...
    long_t value,addr,ret_addr,argA,argB;cond_t cond;alu_t op;
	...
	value=get_reg_val(sim->r,rega);
	addr=get_reg_val(sim->r,regb)+imm;
	if(addr < 0 || addr + 4 > sim->m->len){
	err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, addr);
	return STAT_ADR;
    	}
	set_long_val(sim->m, addr, value);
	sim->pc = next_pc;
	break;
	...
	get_long_val(sim->m,addr,&value);
	set_reg_val(sim->r,rega,value);
	sim->pc = next_pc;
	break;
	...
	op=ifun;
	value=compute_alu(op,argA,argB);
	set_reg_val(sim->r,regb,value);
	sim->cc=compute_cc(op,argA,argB,value);
	sim->pc = next_pc;
	break;
      case I_JMP: /* 7:x imm */
	cond=ifun;
	if(imm < 0 || imm + 4 > sim->m->len){
	err_print("PC = 0x%x, Invalid instruction address 0x%x", sim->pc, imm);
	return STAT_ADR;
    	}
	if(cond_doit(sim->cc,cond))sim->pc=imm;
	else sim->pc = next_pc;
	break;
      case I_CALL: /* 8:x imm */
	if(imm < 0 || imm + 4 > sim->m->len){
	...
	addr=get_reg_val(sim->r,REG_ESP)-4;
	set_reg_val(sim->r,REG_ESP,addr);
	if(addr < 0 || addr + 4 > sim->m->len){
	err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, addr);
	return STAT_ADR;
    	}
	set_long_val(sim->m,addr,next_pc);
	sim->pc = imm;
	break;
      case I_RET: /* 9:0 */
	addr=get_reg_val(sim->r,REG_ESP);
	if(addr < 0 || addr + 4 > sim->m->len){
	err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, addr);
	return STAT_ADR;
    	}
	get_long_val(sim->m,addr,&ret_addr);
